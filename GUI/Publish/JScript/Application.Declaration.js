...
(function () {
  ...
  .controller("application.declarationCtrl", function ($scope, $timeout, $q, utilServ, appServ, pageRouteServ, applicationServ) {
		var self = this;
    ...
    self.resetValue = function () {
      ...
      if (!self.formValue.declarationB)	self.isCheckedDeclarationB = false;
      if (!self.formValue.declarationE) self.isCheckedDeclarationE = false;
    ...
    self.setDeclarationB = function (e, value) {            
			$timeout(function () {
				var isChecked = false;
    ...
    self.setDeclarationE = function (e, value) {            
			$timeout(function () {
				var isChecked = false;
    ...
    self.saveChange = {
      validate: function () {
        ...
        if (!self.formField.declarationASelected) { self.formValidate.isValid.declarationA = false; i++; }
        if (self.formField.declarationASelected === "Y" && !self.formField.declarationB) { self.formValidate.isValid.declarationB = false; i++; }
        if (!self.formField.declarationCSelected) { self.formValidate.isValid.declarationC = false; i++; }
        if (!self.formField.declarationDSelected) { self.formValidate.isValid.declarationD = false; i++; }
        if (!self.formField.declarationESelected) { self.formValidate.isValid.declarationE1 = false; i++; }
        if (self.formField.declarationESelected === "Y" && !self.formField.declarationE) { self.formValidate.isValid.declarationE2 = false; i++; }
