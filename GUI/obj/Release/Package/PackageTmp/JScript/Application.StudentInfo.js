...
(function () {
	...
	.controller("application.studentInfoCtrl", function ($scope, $timeout, $location, $q, utilServ, appServ, dictServ, pageRouteServ, applicationServ, studentCategoryServ, countryServ, universityServ, facultyServ) {
		var self = this;
		...
		self.saveChange = {
			validate: function () {
				...
				if (!self.formField.studentCategorySelected) { self.formValidate.isValid.studentCategory1 = false; i++; }
				if (self.formField.studentCategorySelected && self.formField.studentCategorySpecify[self.formField.studentCategorySelected] !== undefined && !self.formField.studentCategorySpecify[self.formField.studentCategorySelected]) { self.formValidate.isValid.studentCategory2 = false; i++; }
				if (!self.formField.countrySelected.selected) { self.formValidate.isValid.country = false; i++; }
				if (!self.formField.universitySelected.selected && !self.formField.universityOtherSelected) { self.formValidate.isValid.university = false; i++; }
				if (!self.formField.universitySelected.selected && self.formField.universityOtherSelected && !self.formField.universityOther) { self.formValidate.isValid.universityOther = false; i++; }
				if (!self.formField.facultySelected.selected) { self.formValidate.isValid.faculty = false; i++; }
				if (!$scope.datepickerSemester.dateFrom || !$scope.datepickerSemester.dateTo) { self.formValidate.isValid.semester = false; i++; }
