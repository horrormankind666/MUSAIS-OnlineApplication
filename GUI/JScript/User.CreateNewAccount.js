...
(function () {
	...
	.controller("user.createNewAccountCtrl", function ($scope, $timeout, $q, utilServ, appServ, dictServ, titlePrefixServ, genderServ, nationalityServ, countryServ) {
		var self = this;
		...
		self.saveChange = {
			validate: function () {
				...
				if (!self.formField.email) { self.formValidate.isValid.email1 = false; i++; }
				if (self.formField.email && !utilServ.isEmail(self.formField.email)) { self.formValidate.isValid.email2 = false; i++; }
				if (!self.formField.password) { self.formValidate.isValid.password1 = false; i++; }
				if (self.formField.password.length === 1) { self.formValidate.isValid.password2 = false; i++; }
				if (self.formField.password.length > 1 && !self.formField.verifyPassword) { self.formValidate.isValid.verifyPassword1 = false; i++; }
				if (self.formField.verifyPassword && self.formField.password !== self.formField.verifyPassword) { self.formValidate.isValid.verifyPassword2 = false; i++; }
				if (!self.formField.titlePrefixSelected.selected) { self.formValidate.isValid.titlePrefix = false; i++; }
				if (!self.formField.firstName) { self.formValidate.isValid.firstName = false; i++; }
				if (!self.formField.lastName) { self.formValidate.isValid.lastName = false; i++; }
				if (!self.formField.genderSelected) { self.formValidate.isValid.gender = false; i++; }
				if (!self.formField.nationalitySelected.selected) { self.formValidate.isValid.nationality = false; i++; }
