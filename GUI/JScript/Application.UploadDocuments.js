...
(function () {
	...
	.controller("application.uploadDocumentsCtrl", function ($scope, $timeout, $location, $q, utilServ, appServ, dictServ, pageRouteServ, applicationServ) {
		var self = this;
		...
		self.upload = {
			validate: function (document) {
				...
				if (relativeFile.length < 2) { self.formValidate.isValid[document + "1"] = false; i++; }
				if (dictServ.dict.application.table[document].format[utilServ.lang].indexOf(relativeFile[relativeFile.length - 1]) < 0) { self.formValidate.isValid[document + "2"] = false; i++; }
				if (i === 0) {
					var f = new FormData();
						...
						if (result) {
							...
							appServ.upload.action({
								...
								if (result.status) {
									...
									if (document === "passportFile") data = { passportFile: fileName };
									if (document === "facePhotographsFile") data = { facePhotographsFile: fileName };
									if (document === "travelInsuranceFile") data = { travelInsuranceFile: fileName };
									if (document === "transcriptFile") data = { transcriptFile: fileName };
									if (document === "letterHomeUniversityFile") data = { letterHomeUniversityFile: fileName };
									if (document === "curriculumVitaeFile") data = { curriculumVitaeFile: fileName };
									if (document === "TOEFLIELTSFile") data = { TOEFLIELTSFile: fileName };
									if (document === "letterIntentFile") data = { letterIntentFile: fileName };
