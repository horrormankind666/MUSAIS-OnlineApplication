...
(function () {
	...
	.controller("user.editAccounBeforeCtrl", function ($scope, utilServ, appServ, dictServ) {
		var self = this;
	...
	.controller("user.editAccounCtrl", function ($scope, $timeout, $q, utilServ, appServ, dictServ, titlePrefixServ, genderServ) {
		var self = this;
		...
		self.saveChange = {
			validate: function () {
				...
				if (!self.formField.username) { self.formValidate.isValid.username = false; i++; }
				if (self.formField.password.length === 1) { self.formValidate.isValid.password = false; i++; }
